...
def main():
    ...
    print("This script will find all JPG/JPEG files in a directory (and subdirectories)")
    ...
    all_jpg_files = get_jpg_files(target_directory)

    if not all_jpg_files:
        print("\nNo JPG/JPEG files found in the specified directory or its subdirectories.")
        sys.exit(0)
    
    print(f"\nFound {len(all_jpg_files)} JPG/JPEG files.")

    # Perform dry run
    perform_renaming(all_jpg_files, dry_run=True)

    # Ask for confirmation
    confirm = input("\nDo you want to proceed with these changes? (type 'yes' to confirm): ").strip().lower()

    if confirm == 'yes':
        renamed, failed = perform_renaming(all_jpg_files, dry_run=False)
        print("\nOperation Summary:")
        print(f"Total files found: {len(all_jpg_files)}")
